...
function initContactForm() {
  // Ajusté pour correspondre à l'ID réel dans l'HTML (portfolioContactForm)
  const form = document.getElementById('portfolioContactForm') || document.getElementById('contactForm');
  if (!form) return;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const ENDPOINT = form.getAttribute('action') || 'https://formspree.io/f/xoqkqbve';
      ...
      if (res.ok) {
        showSuccess(form, successOverlay);
      } else {
...
function showSuccess(form, successOverlay) {
...
function validateForm(form) {
  ...
  form.querySelectorAll('.form-control, .form-input, .form-textarea').forEach(field => {
  ...
  if (message && message.value.trim().length < 10) {
    showFieldError(message, 'Message trop court (10 caractères min).');
    isValid = false;
  }
